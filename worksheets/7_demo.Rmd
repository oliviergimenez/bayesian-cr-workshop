...
We need to pool the individual encounter histories by unique encounter histories, and recording the number of individuals with a particular unique encounter history. 
```{r}
geese <- read_csv("geese.csv", col_names = TRUE)
y <- as.matrix(geese)
y_weighted <- y %>% 
...
Run nimble. 
```{r}
multisite.marginalized.out <- nimbleMCMC(code = multisite.marginalized, 
                                                     constants = my.constants,
                                                     data = my.data,              
                                                     inits = initial.values(),
                                                     monitors = parameters.to.save,
...
Wow, this run took 1.5 minute, even faster than the marginalized version of the likelihood. 
...
That run took . It would take ages to run the same model on the > 20,000 individual encounter histories. Not even sure it can be run.
