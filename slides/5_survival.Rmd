...
knitr::include_graphics(here::here("slides/img/Marzo_BaguesMance.jpg"))
```
]
]
...
knitr::include_graphics(here::here("slides/img/Marzocuissardes.jpg"))
```
]

---
## 294 birds captured and recaptured between 1981 and 1987 with known sex and wing length
...
<span style="color: red;">A measure of goodness-of-fit of the model to the data</span>: the more parameters you have, the smaller the deviance is (or the bigger the likelihood is).
...
<span style="color: red;">A penalty</span>: twice the number of parameters $K$
...
## Can we describe time variation better?
...
+ Let's use a covariate $\text{sex}$ that takes value 1 if male, and 2 if female
...
+ Think of age for example; covariate or nested indexing works fine <span>&#8212;</span> see live demo
...
+ Assume a model for covariate and fill in missing values (work by S. Bonner, R. Langrock)
...
+ With no information, mean posterior survival is $\phi_{posterior} = 0.56$ 
...
df <- data.frame(posterior = c(phinoprior, phiprior),
                 type = c(rep("vague prior", length(phinoprior)),
                          rep("informative prior", length(phiprior))))
df %>%
  ggplot() + 
