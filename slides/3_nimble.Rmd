...
+ Constants can never be changed and must be provided when a model is defined. E.g., a vector of known index values, variables used in the index ranges of for-loops, etc. 

+ Data can be changed. This allows an algorithm to be applied to many data sets in the same model without re-creating the model each time. It also allows simulation of data in a model.

+ When deciding between data and constants, no reason to use data unless you want the flexibility to change values during your work with the model.
