...
---
### 3 sites Carolinas, Chesapeake, Mid-Atlantic, with 6 years, data kindly provided by Jay Hestbeck ([Hestbeck et al. 1991](https://esajournals.onlinelibrary.wiley.com/doi/10.2307/2937193))
...
---
### HMM model for dispersal with 2 sites
...
---
# What if more than two sites/states?
...
+ Another constraint is that the sum of transition probabilities from any site should be one. 

--

+ Two methods to fullfill both constraints. 
    + Dirichlet prior
...
...
  # Define probabilities of state z(t+1) given z(t)
  gamma[1,1] <- phiA * (1 - psiA[1] - psiA[2])
  gamma[1,2] <- phiA * psiA[1]
  gamma[1,3] <- phiA * psiA[2]
  gamma[1,4] <- 1 - phiA
  gamma[2,1] <- phiB * psiB[1]
  gamma[2,2] <- phiB * (1 - psiB[1] - psiB[2])
  gamma[2,3] <- phiB * psiB[2]
  gamma[2,4] <- 1 - phiB
  gamma[3,1] <- phiC * psiC[1]
  gamma[3,2] <- phiC * psiC[2]
  gamma[3,3] <- phiC * (1 - psiC[1] - psiC[2])
  gamma[3,4] <- 1 - phiC
  gamma[4,1] <- 0
  gamma[4,2] <- 0
  gamma[4,3] <- 0
