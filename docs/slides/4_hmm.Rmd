...
+ Then your state at t depends only on your state at t-1, and it is a multinomial random variable, with one trial, and the probabilities are given by the rows of the transition matrix. 
+ If z at t-1 is alive, it is the first row, that is phi and 1-phi. 
+ Otherwise, if z at t-1 is dead that is 2, then it is the second row of gamma, 0 and 1. If dead you remain dead. 

+ Last we need a prior on survival, as usual we take a uniform distribution between 0 and 1, or a beta with parameters 1 and 1.
