...
\Pr(\mathbf{z}) &= \Pr(z_T, z_{T-1}, z_{T-2}, \ldots, z_1) \color{white}{\Pr(z_{T-1}, z_{T-2},\ldots, z_1) \Pr(z_{T-2}, \ldots, z_1)}\\
                &= \Pr(z_{1}) \prod_{t=2}^T{\gamma_{z_{t-1},z_{t}}}\\
                &= \mathbf{\delta} \mathbf{\Gamma} \ldots \mathbf{\Gamma}
\end{align*}
...
+ In Nimble, we will use the categorical distribution `dcat()` a multinomial distribution with a single draw. 
...
+ For states (in gray), $z = 1$ is alive, $z = 2$ is alive.
...
+ Comment dénoter la matrice des obs? On a $\omega$ et $\delta$ poru les états. Les transitions. 

+ Matrice des observations. Ce sont les émissions. Utiliser T et E instead? 
...
\begin{align*}
\Pr(\mathbf{y}) &= \Pr(y_T, y_{T-1}, y_{T-2}, \ldots, y_1)\\
                &= \Pr(y_T, y_{T-1}, y_{T-2}, \ldots, y_1 | z_T, z_{T-1}, z_{T-2}, \ldots, z_1) \Pr(z_T, z_{T-1}, z_{T-2}, \ldots, z_1)\\
                &= \Pr(z_{1}) \prod_{t=1}^T{\omega_{z_{t}}} \prod_{t=2}^T{\gamma_{z_{t-1},z_{t}}}\\
                &= \mathbf{\delta} \mathbf{\Omega} \mathbf{\Gamma} \cdots \mathbf{\Omega} \mathbf{\Gamma} \mathbf{\Omega} \mathbb{1}
\end{align*}

---
# Example 

+ With alive and dead, and a particular encounter history.
...
True survival is $\phi = 0.8$ and detection is $p = 0.6$.
...
+ Faut-il expliquer que ce sont des données simulées?

+ Probabilities à la main.

+ Histoire de Cormack et Jolly dans même bâtiment. Num spécial Stat science et interviews. 

+ Monographie Lebreton. 

+ state-space which is HMM. Bayes uptake.  
